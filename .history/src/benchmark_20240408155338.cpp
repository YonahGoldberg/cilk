...
void matrixMultiplyParallel(const Matrix &A, const Matrix &B, Matrix &C) {
  for (size_t i = 0; i < A.size(); i++) {
    for (size_t j = 0; j < B.size(); j++) {
      auto result = scheduler.spawn(
          [A, B, C, i, j] { return multiplyCell(A, B, C, i, j); })
    }
  }
  scheduler.sync();
}

Simple n - body problem struct Particle {
  double x, y, z;
  double vx, vy, vz;
  double mass;
};

void calculateForce(const Particle &a, const Particle &b, double &fx,
                    double &fy, double &fz) {
  double G = 9.8;
  double dx = b.x - a.x;
  double dy = b.y - a.y;
  ...
  double distSq = dx * dx + dy * dy + dz * dz +
                  1e-10; // Add small value to prevent division by zero
...
void updateParticle(Particle &a, const double fx, const double fy,
                    const double fz, double deltaTime) {
...
void nBodySimulationParallel(std::vector<Particle> &particles,
                             double deltaTime) {
...
int nqueens(int n, int j, char *a) {
  ...
  scheduler.sync(); // Assuming your scheduler has a way to implicitly wait on
                    // all tasks or futures
