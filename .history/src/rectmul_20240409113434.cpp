...
static long long add_matrix(block *T, long ot, block *R, long orr, long x, long y) {
  ...
  if (x > y) {
    auto future1 = scheduler->spawn([=]() { return add_matrix(T, ot, R, orr, x / 2, y); });
    long long _tmp2 = add_matrix(T + (x / 2) * ot, ot, R + (x / 2) * orr, orr, (x + 1) / 2, y);
    long long _tmp1 = scheduler->steal(future1);
    flops = _tmp1 + _tmp2;
  } else {
    auto future1 = scheduler->spawn([=]() { return add_matrix(T, ot, R, orr, x, y / 2); });
...
void init_matrix(block *R, long x, long y, long o, DTYPE v) {
  ...
  if (x>y) {
    auto future1 = scheduler->spawn([=]() { init_matrix(R, x / 2, y, o, v); });
    init_matrix(R + (x / 2) * o, (x + 1) / 2, y, o, v);
    scheduler->steal(future1);
  } else {
    auto future1 = scheduler->spawn([=]() { init_matrix(R, x, y / 2, o, v); });
...
static long long multiply_matrix(block *A, long oa, block *B, long ob, long x, 
  ...
  if ((x >= y) && (x >= z)) {
    auto future1 = scheduler->spawn([=]() { return multiply_matrix(A, oa, B, ob, x/2, y, z, R, orr, add); });
    ...
    auto future1 = scheduler->spawn([=]() { return multiply_matrix(A, oa, B, ob, x, y, z/2, R, orr, add); });
...
int rectmul(long x, long y, long z) {
  ...
  auto initA = scheduler->spawn([=] { return init_matrix(A, x, y, y, 1.0); });
  auto initB = scheduler->spawn([=] { return init_matrix(B, y, z, z, 1.0); });
  init_matrix(R, x, z, z, 0.0);
  scheduler->steal(initA); // Wait for task to complete
  scheduler->steal(initB); // Wait for task to complete
