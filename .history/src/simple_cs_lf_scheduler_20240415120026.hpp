...
private:
  ...
  std::unordered_map<std::thread::id, int> threadIdToWorker;
...
public:
  ...
  void init(int n) {
    ...
    for ( size_t i = 1; i <= workersCount; ++i )
    {
      ...
      threadIdToWorker[workers[i]->GetThreadId] = workers[i];
...
private:
  ...
  Worker* getWorker() { return threadIds[std::this_thread::get_id()]; }
