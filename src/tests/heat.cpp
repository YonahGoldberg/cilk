...
int divide(int lb, int ub, double **neww, 
  ...
  if (ub - lb > leafmaxcol) {
    int _tmp;
    int r;
    auto fut = scheduler->spawn([=] () {return divide(lb, (ub + lb) / 2, neww, old, mode, timestep);});
    r = divide((ub + lb) / 2, ub, neww, old, mode, timestep);
    int l = scheduler->sync(std::move(fut));
...
int heat(int nxX, int nyX, int ntX,
double xuX, double xoX, double yuX, double yoX, double tuX, double toX,
int leftmaxcolX) {
  nx = nxX;
  ny = nyX;
  nt = ntX;
